...
- set $z_{n+1} = z_{n} \cdot z_{n} + c$
	- if  $|z_{n+1}| < 2 \land n+1 < MAX\_ITERS$
		- ++$n$
		- repeat
	- else
		- break
...
- set $z_{n+1} = z_{n} -\frac{f(z_{n})}{f'({z_{n})}}$
	- if  $\min\limits_{i}(|z_{n+1} - s_{i}|) > \epsilon$ , and $n+1 < MAX\_ITERS$
		- ++$n$
		- repeat
	- else
		- break
