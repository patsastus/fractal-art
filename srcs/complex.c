...
/*   Updated: 2025/06/17 10:28:04 by nraatika         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	multiply complex numbers one and two and store the result in res
	uses a temporary variable so can be used to perform z = z^2, as an example
*/
...
/*
	perform the division res = num / den
*/
...
/*
	sets the value of z given it's pixel coordinates x and y, and the
	value, pixel step size and coordinates given in anchor structure
*/
void	pixel_to_complex(t_complex *z, int32_t x, int32_t y, t_anchor *a)
{
	z->re = a->value.re + (x - a->x) * (a->px_step);
	z->im = a->value.im - (y - a->y) * (a->px_step);
}
